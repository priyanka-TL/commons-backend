...
DEFAULT_STAGE_NAME = "SOLUTIONS"
DEFAULT_BOT_ROUTE = "/fix-solutions-bot"
...
def normalize_solutions_discussed(value: Any) -> Optional[List[str]]:
    """Coerce solutions_discussed to a clean list of strings."""
    if isinstance(value, str):
        try:
...
def extract_solutions_discussed_from_response(response: Any) -> Optional[Any]:
    """Extract solutions_discussed from bot response exactly as returned by mitigation bot."""
    cleaned = unwrap_llm_response(response)

    if isinstance(cleaned, dict):
        solutions_discussed = cleaned.get("solutions_discussed")
        if solutions_discussed is not None:
            return normalize_solutions_discussed(solutions_discussed)
...
def update_story_and_translations(
    ...
    current = other_params.get("solutions_discussed")

    if current != solutions_discussed:
        other_params["solutions_discussed"] = solutions_discussed
    ...
    for tr in translations:
        ...
        if tr_other_params.get("solutions_discussed") != solutions_discussed:
            tr_other_params["solutions_discussed"] = solutions_discussed
...
def process_single_session(
    ...
    """Process one session and update solutions_discussed for guest-discussion story only."""
    result = {
        "session": session_id,
        "status": "skipped",
        "reason": "",
        "story_id": None,
        "solutions_discussed": None,
        "story_updated": False,
        "translations_updated": False,
    }
    ...
    try:
        ...
        relevant_chats = get_solutions_stage_chats(
        ...
        new_solutions_discussed = extract_solutions_discussed_from_response(llm_response)
        if not new_solutions_discussed:
            result["reason"] = "solutions_discussed_not_found_in_bot_response"
            return result

        story_updated, translations_updated = update_story_and_translations(
            story=story,
            solutions_discussed=new_solutions_discussed,
            dry_run=dry_run,
        )

        result["solutions_discussed"] = new_solutions_discussed
...
def process_sessions_from_json(
    ...
    logger.info(
        "Starting solutions_discussed mitigation for %s sessions. dry_run=%s",
        len(session_ids),
        dry_run,
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Mitigate solutions_discussed for guest-discussion stories using /fix-solutions-bot."
    )
    parser.add_argument(
        "--input-json",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Mitigation bot route. Default: /fix-solutions-bot",
    )
    parser.add_argument(
        "--stage-name",
        default=DEFAULT_STAGE_NAME,
        help="Stage name for solutions context. Default: SOLUTIONS",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
