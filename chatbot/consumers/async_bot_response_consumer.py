...
class AsyncBotResponseConsumer(AsyncBaseConsumer):
    ...
    def translate_message(self, message):
        try:
            ...
            if (state_machine and state_machine.text_conversion_type == TextConversionType.TRANSLITERATE and company_chats and len(company_chats)>1):
    ...
    def receive(self, text_data):
        try:
            ...
            if message_type == 'authenticate':
                ...
                self.chat_type = text_data_json.get('chat_type')
                self.bot_route = text_data_json.get('bot_route')
                self.chat_context = text_data_json.get('chat_context')
                self.flow = text_data_json.get('flow')

                if not self.flow:
                    raise Exception("Flow is required")

                if not self.bot_route:
                    raise Exception("Bot route is required")
